...
def train(args):
    ...
    train_dataset = data.GPT2Dataset(traindata, tokenizer)
    valid_dataset = data.GPT2Dataset(validdata, tokenizer)
    ...
    for epoch in range(total_epochs):              
        ...
        for i, (input_id, attention_mask, token_id, lm_label) in enumerate(train_batch):
                  
            model.train()
            input_id, attention_mask, token_id, lm_label = input_id.to(device), attention_mask.to(device), token_id.to(device), lm_label.to(device)              
                  
            train_loss =  model(input_ids = input_id, attention_mask = attention_mask,
                      labels = lm_label, token_type_ids = token_id)[0] 
        ...
        print(f'For epoch : {epoch+1}  Training loss is : {avg_train_loss}  Time taken: {time.time() - start_time}') 
        if (avg_train_loss< loss):
            loss = avg_train_loss
...
if __name__ == "__main__":
    ...
    parser.add_argument('--retraining', type=str, default = 'no' ,
                        help='path to the GPT2 model(default : no)')
